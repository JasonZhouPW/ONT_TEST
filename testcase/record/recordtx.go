...
func TestRecordTransactionByRecord(ctx *TestFrameworkContext) bool {
	...
	recordTx, err := ctx.Ont.NewRecordTransaction(recordType, recordData)
	if err != nil {
		ctx.LogError("NewRecordTransaction RecordType:%s RecordData:%s error:%s", recordType, recordData, err)
		return false
	}

	txHash, err := ctx.Ont.SendTransaction(ctx.OntClient.Account1, recordTx)
	if err != nil {
	...
	recordTx2, err := ctx.Ont.GetTransaction(txHash)
	if err != nil {
...
func TestRecordTransactionByTransfer(ctx *TestFrameworkContext) bool {
	...
	recordTx, err := ctx.Ont.NewTransferAssetTransaction(nil, nil)
	if err != nil {
