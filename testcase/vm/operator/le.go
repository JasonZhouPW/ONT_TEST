...
func TestOperationLargerEqual(ctx *testframework.TestFrameworkContext) bool {
	...
	if !testOperationLargerEqual(ctx, code, 23,2) {
		return false
	}

	if !testOperationLargerEqual(ctx, code, -345, 34){
		return false
	}

	if !testOperationLargerEqual(ctx, code, -10, -234){
		return false
	}

	if !testOperationLargerEqual(ctx, code, 100, 100){
		return false
	}
...
func testOperationLargerEqual(ctx *testframework.TestFrameworkContext, code string, a, b int) bool {
