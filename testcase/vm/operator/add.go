...
func TestOperationAdd(ctx *testframework.TestFrameworkContext) bool {
	...
	if !testOperationAdd(ctx, code, 1, 2) {
		return false
	}

	if !testOperationAdd(ctx, code, -1, 1) {
		return false
	}
	...
	if !testOperationAdd(ctx, code, 0, 0) {
		return false
	}
...
func testOperationAdd(ctx *testframework.TestFrameworkContext, code string, a, b int) bool {
