...
func ParseTransaction(txStr *Transactions) (*transaction.Transaction, error) {
	...
	utxoInputs := make([]*utxo.UTXOTxInput, len(txStr.UTXOInputs))
	...
	outputs := make([]*utxo.TxOutput, len(txStr.Outputs))
	...
	assetOutputs := make(map[common.Uint256][]*utxo.TxOutput, len(txStr.AssetOutputs))
	for _, assetOutput := range txStr.AssetOutputs {
		outputs := make([]*utxo.TxOutput, len(assetOutput.Txout))
...
func ParseTransactionUTXOTxInput(input *UTXOTxInputInfo) (*utxo.UTXOTxInput, error) {
...
func ParseTransactionOutputs(output *TxoutputInfo) (*utxo.TxOutput, error) {
